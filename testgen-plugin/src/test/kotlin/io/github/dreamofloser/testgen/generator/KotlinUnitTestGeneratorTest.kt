...
class KotlinUnitTestGeneratorTest {
    ...
    fun generatesSuspendRepositoryTestWithMockK() {
        ...
        assertTrue(source.contains("import io.mockk.coVerify"))
        ...
        assertTrue(source.contains("assertNotNull(result.getOrNull())"))
        assertTrue(source.contains("assertTrue(result.isFailure)"))
        assertTrue(source.contains("assertEquals(\"sample failure\", result.exceptionOrNull()?.message)"))
        assertTrue(source.contains("coVerify { apiService.getCurrentWeather(any(), any()) }"))
        assertTrue(result.testMethodCount == 2)
        assertTrue(result.assertionCount == 4)
        assertTrue(result.mockVerificationCount == 2)
    }
    ...
    fun generatesRetrofitApiContractTestWithResponse() {
        ...
        assertTrue(source.contains("fun getCurrentWeather_hasRetrofitEndpointMetadata()"))
        assertTrue(source.contains("method.getAnnotation(GET::class.java)"))
        assertTrue(source.contains("assertEquals(\"weather\", annotation!!.value)"))
        assertTrue(source.contains("assertEquals(listOf(\"lat\", \"lon\"), queryNames)"))
        assertTrue(source.contains("fun getCurrentWeather_requestsExpectedUrlWithMockWebServer()"))
        assertTrue(source.contains("val server = MockWebServer()"))
        assertTrue(source.contains("Retrofit.Builder()"))
        assertTrue(source.contains("assertEquals(\"GET\", request.method)"))
        assertTrue(source.contains("assertEquals(\"/weather?lat=1.0&lon=1.0\", request.path)"))
        assertTrue(source.contains("private fun sampleConverterFactory(value: Any): Converter.Factory"))
        assertTrue(source.contains("fun getCurrentWeather_returnsHttpErrorWithMockWebServer()"))
        assertTrue(source.contains("MockResponse().setResponseCode(500)"))
        assertTrue(source.contains("assertFalse(result.isSuccessful)"))
        assertTrue(source.contains("assertEquals(500, result.code())"))
